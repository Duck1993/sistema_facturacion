...
precios_comida = [1.32, 1.65, 2.31, 3.22, 1.22, 1.99, 2.05, 2.65, 4.45]
precios_bebida = [0.25, 0.99, 1.21, 1.54, 1.08, 1.10, 2.00, 1.58, 2.2]
...
def revisar_check():
    # Revisar comida
    for i in range(len(variables_comida)):
        if variables_comida[i].get() == 1:
            cuadros_comida[i].config(state=NORMAL)
            if cuadros_comida[i].get() == '0':
                cuadros_comida[i].delete(0, END)
            cuadros_comida[i].focus()
        else:
            cuadros_comida[i].config(state=DISABLED)
            texto_comida[i].set('0')

    # Revisar bebida
    for i in range(len(variables_bebida)):
        if variables_bebida[i].get() == 1:
            cuadros_bebida[i].config(state=NORMAL)
            if cuadros_bebida[i].get() == '0':
                cuadros_bebida[i].delete(0, END)
            cuadros_bebida[i].focus()
        else:
            cuadros_bebida[i].config(state=DISABLED)
            texto_bebida[i].set('0')

    # Revisar postres
    for i in range(len(variables_postres)):
        if variables_postres[i].get() == 1:
            cuadros_postre[i].config(state=NORMAL)
            if cuadros_postre[i].get() == '0':
                cuadros_postre[i].delete(0, END)
            cuadros_postre[i].focus()
        else:
...
# Panel bebidas
panel_bebidas = LabelFrame(panel_izquierdo, text='Bebida', font=('Dosis', 12, 'bold'),
                           bd=1, relief=FLAT, fg='azure4')
...
# Lista productos
lista_comidas = ['Pollo', 'Cordero', 'Pescado', 'Salmon', 'Arrachera', 'Pizza1', 'Pizza2', 'Pizza3']
lista_bebidas = ['Agua', 'Soda', 'Jugo', 'Cola', 'Cerveza', 'Vino1', 'Vino2', 'Cerveza1']
lista_postres = ['Helado', 'Fruta', 'Brownies', 'Flan', 'Mousse', 'Pastel', 'Pay', 'Pay2']
